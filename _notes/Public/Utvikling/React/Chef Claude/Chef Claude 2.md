...
3. Legg til nøkkelen din i formatet `KEY=VALUE`, uten anførselstegn rundt verdien:
   4. `VITE_HF_ACCESS_TOKEN=din_faktiske_access_token_her`.
5. Legg til `.env` i `.gitignore`, for å forhindre at den lastes opp til GitHub, dette er avgjørende for sikkerheten.
6. Endringer i `.env`-filer krever at du restarter utviklingsserveren for at de nye variablene skal lastes inn. Kjør `npm run dev` på nytt.
7. Du kan nå få tilgang til nøkkelen i React-komponentene dine via `process.env`
